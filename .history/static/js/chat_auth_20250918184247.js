...
(() => {
  ...
  function appendMessage(m, isOwn=false) {
    ...
    head.className = 'head';
    head.textContent = `${m.sender} • ${new Date(m.timestamp).toLocaleString()}`;
    if (isOwn) {
  ...
  function loadHistory(room) {
    fetch(`/history?room=${encodeURIComponent(room)}&limit=200`)
      ...
      .then(arr => {
        ...
        arr.forEach(m => appendMessage(m, m.sender === (window.USERNAME || '')));
      })
      .catch(err => console.error('history err', err));
  }

  // handle room buttons: forms already perform POST join -> reloads page
  // Upload flow
  ...
  uploadBtn.addEventListener('click', () => {
    ...
    fetch('/upload', { method: 'POST', body: fd })
      ...
      }).catch(err => { uploadBtn.disabled = false; console.error(err); alert('Upload failed'); });
  });
  ...
  messageForm.addEventListener('submit', (e) => {
    ...
    // optimistic UI
    appendMessage({ sender: window.USERNAME || 'Me', text, attachment, timestamp: new Date().toISOString() }, true);
    messageInput.value = '';
  ...
  messageInput.addEventListener('input', () => {
    ...
    typingTimer = setTimeout(() => socket.emit('typing', { room: currentRoom, typing: false }), TYPING_TIMEOUT);
  });
