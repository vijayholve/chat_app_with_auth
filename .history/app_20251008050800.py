...
with app.app_context():
    ...
    try:
        # use sqlite3 directly for robust ALTER operations
        ...
        if 'reactions' not in cols:
            try:
                cur.execute("ALTER TABLE message ADD COLUMN reactions TEXT")
                conn.commit()
            except Exception as e:
                print('Could not add reactions column via sqlite3:', e)
        # backfill reactions for existing messages
        try:
            cur.execute("SELECT id, reactions FROM message")
            rows = cur.fetchall()
            for rid, reac in rows:
                if reac is None or reac == '':
                    cur.execute("UPDATE message SET reactions = ? WHERE id = ?", (json.dumps({}), rid))
            conn.commit()
        except Exception as e:
            print('Backfill reactions skipped/error via sqlite3:', e)
        conn.close()
    except Exception as e:
        print('SQLite migration check failed:', e)
...
def handle_all_exceptions(e):
    ...
    try:
        return render_template('error.html', error=str(e), tb=tb), 500
    except Exception:
        # fallback plain text
